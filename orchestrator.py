...
class TeamPackage(BaseModel):
    ...
    new_agents_created: List[Dict[str, str]] = Field(
        default_factory=list,
        description="Details of any new agents that were created"
    )
...
class EngineeringManager:
    ...
    async def orchestrate_goal(self, input_goal: InputGoal) -> TeamPackage:
        ...
        """
        logger.info(f"Starting orchestration for goal: {input_goal.goal_description[:50]}...")
        ...
        try:
            ...
            scouting_report = await self._delegate_to_talent_scout(strategy_doc, input_goal)
            self._track_step("talent_scout", "completed", {"matches": "identified", "gaps": "identified"})
            ...
            if scouting_report.capability_gaps:
                ...
                new_agents = await self._delegate_to_agent_developer(scouting_report, strategy_doc)
                self._track_step("agent_developer", "completed", {"new_agents": len(new_agents)})
            ...
            self._track_step("integration_architect", "completed", {"team_assembled": True})
            ...
            logger.info(f"Orchestration completed successfully for workflow {workflow_id}")
    ...
    async def _delegate_to_systems_analyst(self, input_goal: InputGoal) -> StrategyDocument:
        ...
        return StrategyDocument(
                ideal_roles=[{"role": "analyst", "capabilities": ["analysis"]}],  # Placeholder
    ...
    async def _delegate_to_talent_scout(self, strategy: StrategyDocument, input_goal: InputGoal) -> ScoutingReport:
    ...
    async def _delegate_to_agent_developer(self, scouting_report: ScoutingReport, strategy: StrategyDocument) -> List[Dict[str, Any]]:
        """Delegate new agent creation to the Agent Developer."""
        
        from agents.agent_developer import AgentDeveloper, ScoutingReport as DevScoutingReport, VacantRole
        ...
        for role_data in strategy.ideal_roles:
            vacant_role = VacantRole(
                ...
                core_responsibilities=role_data.get("responsibilities", ["Handle assigned tasks"]),
                required_capabilities=role_data.get("capabilities", ["Basic task execution"]),
                interaction_patterns=role_data.get("interactions", {"team": "collaborative"}),
                success_metrics=role_data.get("metrics", ["Task completion", "Quality standards"]),
        ...
        if generation_result.success:
            logger.info(f"Successfully created {len(generation_result.agents_created)} agents")
            ...
            logger.warning(f"Agent generation failed: {generation_result.generation_summary}")
            return []
    ...
    async def _delegate_to_integration_architect(
        ...
        from agents.integration_architect import IntegrationArchitect, StrategyDocument as ArchitectStrategy, ScoutingReport as ArchitectScouting, NewAgent
        
        architect = IntegrationArchitect(db_file=f"integration_architect_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db")
        
        # Convert internal data structures to architect's expected formats
        architect_strategy = ArchitectStrategy(
            goal_analysis={"complexity": input_goal.complexity_level, "domain": input_goal.domain},
            team_composition=strategy.ideal_roles,
            team_structure={"topology": "hierarchical", "coordination": "workflow_based"},
            risk_assessment=["Integration complexity", "Communication overhead"],
            resource_requirements={"agents": len(strategy.ideal_roles), "coordination": "high"},
        ...
        architect_new_agents = [
            NewAgent(
                ...
                system_prompt=agent.get("prompt", f"You are a {agent.get('role', 'specialist')} agent..."),
        ...
        doc_path = architect.create_roster_documentation(
            ...
            f"AgentForge Team - {input_goal.domain}",
            "Roster.md"
        )
        ...
        return TeamPackage(
            ...
            team_members=[
                # Existing agents
                *[{"role": agent["name"], "type": "existing", "capabilities": agent.get("capabilities", []), "source": "agent_library"} 
                  for agent in scouting.matched_agents],
                # New agents
                *[{"role": agent.name, "type": "new", "capabilities": agent.capabilities, "source": "agent_developer"} 
            ...
            new_agents_created=[{"name": agent.name, "role": agent.role, "capabilities": agent.capabilities} for agent in architect_new_agents],
            existing_agents_used=[{"name": agent["name"], "match_score": agent.get("match_score", 0.0)} for agent in scouting.matched_agents],
            ...
            success_metrics=input_goal.success_criteria or ["Goal completion", "Quality standards", "Timeline adherence", "Team coordination efficiency"]
        )
    
    async def _finalize_team_package(self, team_package: TeamPackage, input_goal: InputGoal) -> TeamPackage:
    ...
    async def run_with_goal(self, goal_description: str, domain: str = "general", **kwargs) -> TeamPackage:
